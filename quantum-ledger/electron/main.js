...
app.whenReady().then(async () => {
  ...
  // If a server is already listening on PORT (e.g. npm run dev:server), skip
  // starting a second instance so both can coexist without a port conflict.
  const serverAlreadyUp = await isReachable(`http://127.0.0.1:${PORT}/api/health`);
