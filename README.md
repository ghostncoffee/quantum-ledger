...
All `package.json` / `package-lock.json` files across the repo share a single version number. To bump it everywhere at once:

```bash
node scripts/set-version.js 1.2.0
```

Review the diff, update the relevant CHANGELOG(s), then commit.
